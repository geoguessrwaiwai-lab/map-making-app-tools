...
8. ぽちぽちモードをONにした時点のロケーションは、次へ移動しても残る
9. ON中に初めて選択したロケーションは、次へ移動したときだけ削除される
10. 過去に選択済みのロケーションへ戻った場合は削除されない
11. ON中にSaveまたはCloseしたロケーションを再選択しても削除されない
12. オプション画面で2機能を個別に無効化できる
13. URL別設定がないマップでは、オプション画面のデフォルトON／OFFが反映される
